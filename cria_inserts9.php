...
function gerar_nome_compacto($servico, $publico, $valor, $arquivo, $i) {
    $raw = $servico . '|' . $publico . '|' . $valor . '|' . $arquivo . '|' . $i;
    return substr(hash('sha256', $raw), 0, 32); // 32 chars (128 bits) codificados em hex
}
...
function gerar_inserts_para_csv($caminho_csv, $saida) {
    ...
    while (($linha = fgetcsv($handle, 0, '|')) !== false) {
        ...
        $chave_servico = mb_strtolower(trim($valor_servico . '||' . $valor_publico . '||' . $nome_arquivo), 'UTF-8');

        if (!isset($servicos_registrados[$chave_servico])) {
            fwrite($saida,
                "INSERT IGNORE INTO servicos (nome_servico, id_area_mte, id_pessoa_alvo, id_arquivo_fonte) VALUES (\n" .
                "  \"$valor_servico\",\n" .
    ...
    foreach ($linhas_buffer as $dados) {
        ...
        foreach ($linha as $i => $valor) {
            ...
            $nome_campo_servico = gerar_nome_compacto($valor_servico, $valor_publico, $valor, $nome_arquivo, $i);
            ...
            fwrite($saida,
                ...
                "  (SELECT id_chave_servico FROM servicos WHERE nome_servico = \"$valor_servico\" AND id_pessoa_alvo = (SELECT id_chave_pessoa_alvo FROM pessoas_alvo WHERE nome_pessoa_alvo = \"$valor_publico\") AND id_arquivo_fonte = (SELECT id_chave_arquivo_fonte FROM arquivos_fonte WHERE nome_arquivo_fonte_CSV = \"$nome_arquivo\"))\n" .
                ");\n"
            );
        }
    }
