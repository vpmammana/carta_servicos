...
function gerar_inserts_para_csv($caminho_csv, $saida) {
    ...
    // Leitura das linhas: gera valores_campos e servicos
    while (($linha = fgetcsv($handle, 0, '|')) !== false) {
        ...
        // INSERT servicos com id_arquivo_fonte
        fwrite($saida,
        ...
        // Armazena para posterior geração de campos_servicos
        $linhas_buffer[] = [
            'valores' => $linha,
            'cabecalho' => $cabecalho,
    ...
    // INSERTs campos_servicos (depois dos serviços)
    foreach ($linhas_buffer as $dados) {
        ...
        foreach ($linha as $i => $valor) {
            ...
            $nome_campo_servico = "{$valor_servico}_{$valor_publico}_{$campo}_$i";
            ...
            fwrite($saida,
                ...
                "  (SELECT id_chave_servico FROM servicos WHERE nome_servico = \"$valor_servico\" AND id_arquivo_fonte = (SELECT id_chave_arquivo_fonte FROM arquivos_fonte WHERE nome_arquivo_fonte_CSV = \"$nome_arquivo\"))\n" .
                ");\n"
            );
        }
    }
